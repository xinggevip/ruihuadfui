<template>
  <div class="dafen">
    ...
    <div v-if="active == 3">
        ...
        <van-form @submit="onSubmitOfUpdateCode">
          <van-field
            v-for="(item,index) in invitaionList" 
            :key="index"
            type="number"
            v-model="invitaionList[index].invitationCode"
            :name="parserCodeName(item.invitationType)"
            :label="parserCodeName(item.invitationType)"
            :placeholder="parserCodeName(item.invitationType)"
            :rules="[{ required: true, message: '' }]"
          />
    ...
    <div class="btn-father">
      ...
      <van-button v-if="active == 3" @click="end" plain  round type="info" style="width:100%;margin-top:30px;">完成</van-button>
    </div>
...
export default {
  ...
  methods:{
    ...
    onSubmitOfUpdateCode(values) {
      console.log('submit', this.invitaionList);
      this.$toast.loading({
            message: '加载中...',
            forbidClick: true,
          });
      this.$post("/invitation/updateInvitations",this.invitaionList).then(response=>{
        if(response.data.success){
